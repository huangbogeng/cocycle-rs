# Historical Python-wrapper tools

These retained Python-wrapper benchmarks are for historical reproduction.
Use [native benchmarks](../benches/native/README.md) for current C++ comparisons.
...
including nested pages under `docs/`. Run its standard-library regression checks
with `python3 -m unittest discover -s tools -p 'test_check_docs.py'`.
...
[mathematics section 10](../docs/reference/mathematics.md#10-analytic-complete-bipartite-filtration).
Validation compares endpoint multiplicities, not only
