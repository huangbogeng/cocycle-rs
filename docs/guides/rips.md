...
Cocycle currently computes ordinary Vietoris–Rips persistence over F2 in H0 or
H0/H1. Input buffers are borrowed; returned diagrams own their data and outlive
the input. See the [mathematical specification](../reference/mathematics.md) for exact conventions.
...
are rejected explicitly. There is no sampling or approximation switch.
...
Point-cloud distance storage is quadratic in the number of vertices. H1 uses
...
the caller controls execution budgets. Rust allocation failures are not uniformly
